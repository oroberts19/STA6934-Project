...
wdi <- wdi2[-(1:2),] 
wdi <- wdi %>% as.data.frame() %>%  mutate_each(funs(as.numeric)) %>% mutate_each(funs(as.factor))
...
The correlation between the share of youth that are not employed, in education or job training and the overall unemployment rate across time is 0.84 The strength of this correlation, as well as the clear linear trend depicted in the scatterplot provided, motivates us to further study the plausibility of a significant relationship between the two variables.  
...
```{r}
plot(regression_data$share_youth_NEET, regression_data$unemployment_total) 
abline(lm_fit)
```
...
The interpretation of the predictor is as follows: for a 1% increase in consecutive yearly change in % of youth not in education or employment training, consecutive yearly change in unemployment rate increases by 0.969% on average.    
